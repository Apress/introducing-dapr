    ...
    public static class WorkflowCreator
    {
        public static IEnumerable<WorkflowConfig> LoadWorkflows(string workflowsDir, EdgeFlowWebManagementEngine engine)
        {
            ...
            foreach (var file in Directory.EnumerateFiles(workflowsDir, "*.json", SearchOption.AllDirectories))
            {
                ...
                var workflowDefinition = JsonConvert.DeserializeObject<FlowPropertiesDefinition>(workflowJson);
                
                var def = new FlowDefinition(FlowConstants.GeneralAvailabilitySchemaVersion);
                def.Properties = workflowDefinition;

                var flowName = Path.GetFileNameWithoutExtension(fi.Directory.Name);
